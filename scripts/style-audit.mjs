/* -----------------------------------------------------------------------------
 * style-audit.mjs — deterministic Playwright style checks for the Product
 * Comparison tab. Invoked by scripts/gate.py; not meant to be run directly in
 * normal workflows. Exits 0 with no output on PASS; on failure prints one line
 * per violation and exits 1 (gate.py forwards those lines).
 *
 * Checks (Alexa's anti-box law, enforced as geometry):
 *   1. Ledger cells: the two product columns are equal width to each other and
 *      uniform row-to-row; every ledger cell is the same height.
 *   2. Zero bordered box nested inside another bordered box within the tab
 *      (a "box" = borders on >=3 sides and NOT a circle/pill; 1-2 sided borders
 *      are hairline rules and allowed).
 *   3. <=2 distinct non-zero border-radius values in the tab.
 *   4. No horizontal scroll at 1440px and at 375px.
...
async function openTab(page) {
  ...
  await page.getByRole('button', { name: /Product Comparison/i }).first().click();
  await page.getByText('What ships in each product').first().waitFor({ timeout: 15000 });
  await page.waitForTimeout(300);
}

// DOM-side audit of the tab geometry / boxes / radii. Returns a list of problem strings.
function tabProbe() {
  const out = [];
  const tab = document.querySelector('[data-tab="product-comparison"]');
  if (!tab) return ['comparison tab container [data-tab="product-comparison"] not found'];
  ...
  // (3) distinct non-zero border-radius values
  const radii = new Set();
...
async function run() {
  ...
  try {
    // --- 1440px, light: geometry + boxes + radii + horizontal scroll + banner ---
    {
      const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 }, colorScheme: 'light' });
      const page = await ctx.newPage();
      await openTab(page);
      for (const p of await page.evaluate(tabProbe)) problems.push(`[1440] ${p}`);
      const overflow = await page.evaluate(noHorizontalScroll);
      if (overflow > 1) problems.push(`[1440] horizontal scroll: scrollWidth exceeds clientWidth by ${overflow}px`);
      const banner = await page.evaluate(draftBannerState);
      if (!banner.ok) problems.push(`[1440 light] draft banner not visibly distinct: ${JSON.stringify(banner)}`);
      await ctx.close();
    }
    // --- 1440px, dark: banner visibility in the second theme ---
    {
      const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 }, colorScheme: 'dark' });
      const page = await ctx.newPage();
      await openTab(page);
      const banner = await page.evaluate(draftBannerState);
      if (!banner.ok) problems.push(`[1440 dark] draft banner not visibly distinct: ${JSON.stringify(banner)}`);
      await ctx.close();
    }
    // --- 375px, light: no horizontal scroll on mobile ---
    {
      ...
      await openTab(page);
      const overflow = await page.evaluate(noHorizontalScroll);
      if (overflow > 1) problems.push(`[375] horizontal scroll: scrollWidth exceeds clientWidth by ${overflow}px`);
      await ctx.close();
    }
  } finally {
    await browser.close();
