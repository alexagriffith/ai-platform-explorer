...
4. UI changes: browser-verify the affected tab(s); check dark mode for anything touching backgrounds (theming is class-based `html.dark`, toggled via the header button; explicit choice persisted to `localStorage`; when no explicit choice, follows `matchMedia('prefers-color-scheme: dark')` — flip the header toggle or emulate dark in devtools).
